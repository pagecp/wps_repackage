...
First activate the ``wps_repackage`` Conda environment and install ``pytest``.
...
.. code-block:: console
  ...
  $ make clean
  $ make install
  $ conda list -n wps_repackage --explicit > spec-file.txt
...
Make a new version of wps_repackage in the following steps:
